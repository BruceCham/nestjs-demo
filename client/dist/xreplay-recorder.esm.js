...
function getExtra(node, isSVG) {
  ...
  if (isSVG || tagName.toLowerCase() === 'svg') {
    ...
    const rules =
      (_b = (_a = node) === null || _a === void 0 ? void 0 : _a.sheet) ===
        null || _b === void 0
        ? void 0
        : _b.rules;
...
const createElement = (el, inheritSVG) => {
  ...
  if (
    (_c =
      (_b = (_a = vNode) === null || _a === void 0 ? void 0 : _a.extra) ===
        null || _b === void 0
        ? void 0
        : _b.props) === null || _c === void 0
      ? void 0
      : _c.textContent
  ) {
    return vNode;
...
class Watcher {
  constructor(options) {
    ...
    this.recordOptions =
      context.G_RECORD_OPTIONS || window.G_RECORD_OPTIONS || {};
    this.emit = emit;
    this.init(options);
  }
  init(options) {}
...
function rewriteNodes(vNodes, configs, preFetchCallback) {
  ...
  if (
    ...
    return logError(
      'The params replaceOrigin and matches is required for using rewriteResource',
    );
  }
  ...
  rewriteConfigs.forEach((config) => {
    ...
    const rewriteAttr = (vNode, key, source) => {
      ...
      const url = createURL(
        source,
        (base === null || base === void 0 ? void 0 : base.href) || href,
      );
  ...
  preFetchConfigs.forEach((config) => {
    ...
    const rewriteNode = (node) =>
      matchNodeSource(node, matches)(preFetchSource);
    const preFetchSource = (vNode, key, source) =>
...
class DOMWatcher extends Watcher {
  ...
  waitAndRecordIFrame(iframe) {
    ...
    (_b =
      (_a = iframe) === null || _a === void 0 ? void 0 : _a.frameRecorder) ===
      null || _b === void 0
      ? void 0
      : _b.destroy();
    const onLoadHandle = () => {
  ...
  rewriteAddedSource(addedNodes, time) {
    ...
    const vNodes = addedNodes
      .map((item) => item.node)
      .filter((node) => isVNode(node) && node);
  ...
  listenInputs(options) {
    ...
    Object.values(eventTypes)
      .map((type) => (fn) => {
        ...
        this.uninstall(() =>
          context.removeEventListener(type, fn, eventListenerOptions),
        );
      })
      ...
      switch (eventType) {
        ...
        case eventTypes.change:
          ...
          data = {
            type:
              eventType === 'input'
                ? FormElementEvent.INPUT
                : FormElementEvent.CHANGE,
...
class LocationWatcher extends Watcher {
  constructor() {
    ...
    this.locationHandle = (e) => {
      ...
      const [, , path] = e.arguments || [
        ...
        ,
        (_b =
          (_a = this.context) === null || _a === void 0
            ? void 0
  ...
  init() {
    this.context.history.pushState = this.kidnapLocation(
      LocationTypes.pushState,
    );
    this.context.history.replaceState = this.kidnapLocation(
      LocationTypes.replaceState,
    );
    const types = Object.values(LocationTypes);
    types.forEach((type) =>
      this.toggleListener(MethodType.add, type, this.locationHandle),
    );
    this.uninstall(() => {
      types.forEach((type) =>
        this.toggleListener(MethodType.rm, type, this.locationHandle),
      );
    });
  }
  toggleListener(methodType, type, handle) {
    this.context[
      methodType === MethodType.add ? 'addEventListener' : 'removeEventListener'
    ](type, handle);
  }
  kidnapLocation(type) {
...
function attr(input) {
  const attrs = Array.from(input.attributes).filter((attr) =>
    config$1.attr(attr.name, attr.value),
  );
...
class MouseWatcher extends Watcher {
    ...
    const evt = (e) => {
      ...
      if (offsetPosition) {
        this.emitData(
          RecordType.MOUSE,
          Object.assign(
            { type: MouseEventType.CLICK, selector },
            offsetPosition,
          ),
        );
      }
  ...
  getOffsetPosition(event, context) {
    ...
    if (view === context) {
      ...
      const frameElement =
        (_a = doc === null || doc === void 0 ? void 0 : doc.defaultView) ===
          null || _a === void 0
          ? void 0
          : _a.frameElement;
...
class ScrollWatcher extends Watcher {
  ...
  wrapData(target, isAuto = false) {
    const element =
      target instanceof this.context.HTMLElement
        ? target
        : this.getCompatibleTarget(target);
    const data = {
...
const watchers = Object.assign(
  Object.assign({ LocationWatcher }, baseWatchers),
  { WindowWatcher, FontWatcher, TerminateWatcher },
);
  ...
  constructor(options) {
    ...
    this.checkHookAvailable = () => {
      try {
        ...
        logError(
          `Plugin hooks is not available in the current env, because ${error}`,
        );
      }
    };
    this.plugin = (type, cb) => {
...
class RecorderModule extends Pluginable {
  constructor(options) {
    ...
    this.watchesReadyPromise = new Promise(
      (resolve) => (this.watcherResolve = resolve),
    );
    this.status = RecorderStatus.PAUSE;
  ...
  initOptions(options) {
    const opts = Object.assign(
      Object.assign({}, RecorderModule.defaultRecordOpts),
      options,
    );
    if (opts.video === true) {
      opts.video = { fps: 24 };
  ...
  startRecord(options) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const onEmit = (options) => {
        const emitTasks = [];
        const { middleware: rootMiddleware } = this.options.rootRecorder || {
          middleware: [],
        };
      ...
      const relatedId = isInRoot
        ? headData.relatedId
        : options.rootContext.G_RECORD_RELATED_ID;
      ...
      if (isInRoot && options.emitLocationImmediate) {
        const locationInstance = this.watchersInstance.get(
          LocationWatcher.name,
        );
        locationInstance === null || locationInstance === void 0
          ? void 0
